...
var MainManager = function() {
	...
	function t() {
		g = 1, $("#scene_loading hr").css("display", "none"), $("#scene_loading hr").css("width", 0), $("#scene_loading").stop().fadeOut(200, "linear"), v ? ($("#scene_loading").stop().css("display", "none"), $("#bt_back").stop().css("display", "none"), p && $("#bt_fs").stop().css("display", "none"), $("#scene_main .set").stop().css("display", "none")) : $("#scene_main").stop().fadeIn(200, "linear"), I = aidn.___waContext.currentTime, k.start(), P.start()
	}
	...
	function o() {
		if (P.update(), 1 == g && 0 > --L && r(), v && 1 == g) {
			...
			if (l * s < a) {
				...
				if (h += e - l, 0 <= (l = e) * s - a)
					for (e = Math.random(), a = 1, 192 <= h ? h = 0 : 128 <= h ? (.7 > e && (a = 2), .5 > e && (a = 3)) : 64 <= h ? (.35 > e && (a = 2), .2 > e && (a = 3), .02 > e && (a = 0)) : 32 <= h ? (.35 > e && (a = 2), .24 > e && (a = 0)) : 0 <= h && .4 > e && (a = 0), e = 0; e < a; e++) d = c[f], 32 <= ++f && (f = 0, aidn.util.shuffleArray(c)), k.changeId(d, 0, !0)
			}
		}
	...
	aidn.util.useDummyDiv(), this.init = function() {
		aidn.window.addDummyDiv();
		var e = 1;
		2 <= window.devicePixelRatio && (e = 2), (x = PIXI.autoDetectRenderer(w, M, {
			backgroundColor: 16756655,
			antialias: !1,
			resolution: e
		})).autoResize = !0, document.getElementById("view").appendChild(x.view), b = new PIXI.Container, k.init(), a(), $("#scene_top").fadeIn(300), o()
	};
	...
	var w, M, I, x, b, C = 0,
		...
		P = new function() {
			...
			this.init = function(t, o) {
				...
				(n = new WebAudioManager).load("js/data/track/track.json", r, a, e)
			...
			this.init = function(n, t) {
				...
				(i = new WebAudioManager).load("js/data/main/main.json", r, a, e)
			...
			var i, o = 32;
			this.length = o;
			for (var r = [], l = [], s = 0; s < o; s++) r[s] = 1, l[s] = .05;
			...
			function t(e) {
				...
				if (r(n), e.originalEvent && e.originalEvent.touches)
					for (var t = e.originalEvent.touches.length, i = 1; i < t; i++) n = e.originalEvent.touches[i], n = a(n.pageX, n.pageY), r(n, 1)
			}
			...
			var d = function(a, e) {
				this.id = a, this.setPosition = function(a, e) {
				...
				r.interactive = !0, e.addChild(r)
					...
					function n() {
						...
						var e, n;
						if (0 == o)
							for (var t = 0; t < h.rotation; t += 30) e = (s * t + l) * Math.PI / 180, n = Math.cos(e) * a, e = Math.sin(e) * a, c.lineTo(n, e);
						else
							for (t = 360; h.rotation < t; t -= 30) e = (s * t + l) * Math.PI / 180, n = Math.cos(e) * a, e = Math.sin(e) * a, c.lineTo(n, e);
						e = (s * h.rotation + l) * Math.PI / 180, n = Math.cos(e) * a, e = Math.sin(e) * a, c.lineTo(n, e), c.lineTo(0, 0), c.endFill()
					}

					function t() {
						o = 1, TweenLite.fromTo(h, .9, {
							rotation: 0
						}, {
							rotation: 360,
							ease: Power1.easeOut,
					...
					this.play = function(a, e) {
						o = 0, d = a, r = e, l = 360 * Math.random(), s = 1, .5 > Math.random() && (s = -1), c.clear(), c.beginFill(0), c.moveTo(0, 0), c.lineTo(1, 1), c.endFill(), TweenLite.fromTo(h, .6, {
							rotation: 0
						}, {
							rotation: 360,
							ease: Power1.easeOut,
					...
					var t = function(a) {
						...
						this.play = function(a, f, u, v) {
							return t.visible = !0, c = 0, i = a, o = f, l = u, s = v, d = .2 * Math.random() + .2, h = .2 * Math.random() + .2, r = {
								x: i.x,
								y: i.y
							}, TweenLite.to(r, d, {
								x: o.x,
					...
					this.play = function(a) {
						r = a, o.setChildIndex(s, o.children.length - 1), s.visible = !0, s.x = w / 2, s.y = M / 2, s.rotation = .5 * Math.PI * Math.floor(4 * Math.random()), a = Math.floor(7 * Math.random() + 2);
			...
			this.resize = function() {
				...
				C = !0, X = new PIXI.Container, b.addChild(X), u = new PIXI.Container, b.addChild(u), (z = new function(a) {
					function e() {
					...
					this.resize = function() {
						...
						for (var a = 0; 3 > a; a += 2) TweenLite.delayedCall(.07 * a, e), TweenLite.delayedCall(.07 * (a + 1), n)
				...
				m || ($("#view").on("mousedown", t), $(window).on("mousemove", i), $(window).on("mouseup", o), $(window).on("keydown", e), $(window).on("keyup", n)), (m || window.TouchEvent) && ($("#view").on("touchstart", t), $(window).on("touchmove", i), $(window).on("touchend", o)), $("#view").css("cursor", "pointer")
			}, this.end = function() {
				m || ($("#view").off("mousedown", t), $(window).off("mousemove", i), $(window).off("mouseup", o), $(window).off("keydown", e), $(window).off("keyup", n)), (m || window.TouchEvent) && ($("#view").off("touchstart", t), $(window).off("touchmove", i), $(window).off("touchend", o)), $("#view").css("cursor", "auto")
			...
			var u, v = -1,
				...
				P = [
					function(a, e) {
						...
						var t = function(a) {
							...
							this.play = function(a, t, i) {
								...
								return n.lineStyle(3 * Math.random() + 3, i), n.drawCircle(0, 0, l), n.x = a, n.y = t, n.scale.x = 0, n.scale.y = 0, n.rotation = Math.random() * Math.PI, a = .2 * Math.random() + .4, TweenLite.to(n, a, {
									x: o,
									y: r,
					...
					function(a, e) {
						...
						var t = function(a) {
							...
							this.play = function(a, t, i) {
								...
								return n.beginFill(i), n.drawRect(-l / 2, -l / 2, l, l), n.x = a, n.y = t, n.scale.x = 0, n.scale.y = 0, n.rotation = Math.random() * Math.PI, a = .2 * Math.random() + .4, TweenLite.to(n, a, {
									x: o,
									y: r,
									rotation: Math.random() * Math.PI,
									ease: Power3.easeOut,
					...
					function(a, e) {
						...
						var t = function(a) {
							function e() {
								l.beginFill(i), l.drawCircle(0, 0, o), l.scale.x = 0, l.scale.y = 0, TweenLite.to(l.scale, .7, {
									x: 1,
									y: 1,
						...
						this.play = function() {
							...
							for (var a = 10, e = l(), i = Math.min(w, M) / 64 * (.6 * Math.random() + .7), d = 2, h = 0; 40 > h; h++) {
								var c, f = 25 * h * Math.PI / 180,
									u = a * Math.cos(f);
								f = a * Math.sin(f), a = a + i, d = d + .22;
								c = r[h] ? r[h] : new t(s);
								var v = null;
								39 == h && (v = n), c.play(.03 * h, e, d, u, f, v)
							}
						};
						var i = this,
							o = a;
					...
					function(a, e) {
						...
						this.play = function() {
							...
							t = .8 * Math.random() + .4, a = .8 * Math.random() + .4, TweenLite.fromTo(r.scale, .9, {
								x: t,
								y: t
							}, {
								x: a,
					...
					function(a, e) {
						...
						var t = function(a) {
							function e() {
								...
								r.lineStyle(4 * Math.random() + 4, l()), r.drawRect(-e / 2, -e / 2, e, e), r.x = i + a / 2 * (Math.random() - .5), r.y = o + a / 2 * (Math.random() - .5), r.scale.x = 0, r.scale.y = 0, r.rotation = Math.random() * Math.PI, TweenLite.to(r, .5, {
									x: i,
									y: o,
									rotation: 0,
									ease: Back.easeOut.config(1.7),
					...
					function(a, e) {
						...
						var t = function(a) {
							function e() {
								...
								r.beginFill(l()), r.drawCircle(0, 0, a), r.x = i, r.y = o, r.scale.x = 0, r.scale.y = 0, TweenLite.to(r.scale, .5, {
									x: 1,
									y: 1,
									ease: Elastic.easeOut.config(1, .3),
									onComplete: n
					...
					function(a, e) {
						...
						this.play = function() {
							...
							a = .3 * Math.random() + .6, TweenLite.fromTo(o.scale, a, {
								x: 0,
								y: 0
							}, {
								x: r,
					...
					function(a, e) {
						...
						var t = function(a) {
							...
							this.init = function(a, e, n, t) {
								...
								t = n, i.clear(), i.visibloe = !0, i.beginFill(l), i.drawRect(.5 * -o, .5 * -o, o, o), TweenLite.fromTo(i.scale, .3, {
									x: 0,
									y: 0
								}, {
									x: 1,
						...
						this.play = function() {
							...
							for (var u = 0; u < a; u++) {
								...
								v = s[u] ? s[u] : new t(r);
								var y = null;
								u == a - 1 && (y = n), v.init(m, p, d, h), v.play(.05 * u, y)
							}
						};
						var i = this,
							o = a;
					...
					function(a, e) {
						...
						var t = function(a) {
							...
							this.init = function(a, e, n, t) {
								...
								t = n, i.clear(), i.visibloe = !0, i.beginFill(r), i.drawCircle(0, 0, .5 * o), TweenLite.fromTo(i.scale, .3, {
									x: 0,
									y: 0
								}, {
									x: 1,
					...
					function(a, e) {
						...
						this.play = function() {
							o.visible = !0, i.setChildIndex(o, i.children.length - 1), o.x = .2 * w + .6 * w * Math.random(), o.y = .2 * M + .6 * M * Math.random();
							var a = Math.min(w, M) * (.7 + .2 * Math.random()),
								e = a / 10 * (.5 + .8 * Math.random()),
								t = l();
							r.clear(), r.beginFill(t), r.drawRect(0, -e / 2, a, e), s.clear(), s.beginFill(t), s.drawRect(-e / 2, 0, e, a), r.y = 0, r.x = -a / 2, s.x = 0, s.y = -a / 2, o.rotation = 45 * Math.PI / 180, r.scale.x = 0, s.scale.y = 0, a = .5 > Math.random() ? -135 * Math.PI / 180 : 215 * Math.PI / 180, (new TimelineLite).to(r.scale, .4, {
								x: 1,
								ease: Power2.easeOut
							}).to(s.scale, .4, {
								y: 1,
					...
					function(a, e) {
						...
						this.play = function() {
							f.clear(), f.visible = !0, .5 > Math.random() ? (f.x = 0, f.y = 0, f.rotation = 0) : (f.x = w, f.y = M, f.rotation = Math.PI), h = o = 0, i = Math.floor(3 * Math.random()) + 3, r = 20 * Math.random() + 2, s = l();
				...
				k = [
					function(a, e) {
						...
						this.play = function() {
							D = c;
							var a = s();
							r = E[a], $("#about").css("background-color", "#" + r.toString(16)), .3 > Math.random() && z.flash(d), A = a, h.clear(), h.visible = !0, d = o.children.length - 1 - Math.floor(2 * Math.random()), o.setChildIndex(h, d);
							a = .5 > Math.random();
							...
							for (var u = l = 0; u <= e; u++) {
								...
								0 != u && u != e && (p = f / 4 * Math.random() - f / 8), a ? v.y = f * u + p : v.x = f * u + p, i.pos["p" + u] = v, v = .4 * Math.random() + .3, l = 2, TweenLite.to(i.pos["p" + u], v, a ? {
									x: w
								} : {
									y: M
								})
							}
							i.progress = 0, TweenLite.to(i, l, {
								progress: 1,
	...
	i(), aidn.util.webaudio ? ($("#ng").css("display", "none"), $(".ok").css("display", "block"), m && $("#scene_main .attention").html("轻触 &amp; 滑动!"), u || $("#scene_top .attention").text("* 调节好音量，和MIKU一起学习吧! ")) : ($("#ng").css("display", "block"), $(".ok").css("display", "none"), u || $("#ng .atten").html("抱歉，</br>你的浏览器不支此页面")), PIXI.utils._saidHello = !0, aidn.window.resize(a)
		...
		this.load = function(l, s, d, h) {
			t = d, i = h, n = s, e = s.length, r.length = e, $.getJSON(l, function(e) {
				o = e, a()
			})
